...
That first line is the systemd form. `openclaw-proton-pass setup` writes the
service file this host actually uses, with the install's paths already
resolved, and prints the command that enables it. [Running it as a
service](/guides/running-as-a-service) covers macOS, Windows and containers.
