...
Stream an upstream `Response` body to a client socket.

**Signature**

```ts
export declare const relayUpstreamResponse: (
...
assert.strictEqual(Effect.isEffect(relayUpstreamResponse(upstream, client)), true)
```
