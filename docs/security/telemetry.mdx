...
<Note>
This page is about telemetry, which is a different channel from the vault's
own audit log. Every audited read does send Proton a reason naming the secret
map ids involved, because that log exists to say what was read. See [sessions
and the agent token](/concepts/sessions). Nothing from that reason reaches
telemetry, and nothing here reaches Proton.
</Note>
